...
public class TimedInterface {
    private static class TimedHandler<T> implements InvocationHandler, CompositeMonitor<Long> {
        ...
        TimedHandler(Class<T> ctype, T concrete, String id) {
            ...
            MonitorConfig.Builder configBuilder = MonitorConfig.builder(ctype.getSimpleName())
                    .withTag("class", concrete.getClass().getSimpleName());

            if (id != null) {
                configBuilder.withTag("id", id);
            }
            ...
            timers = CacheBuilder.newBuilder().build(new CacheLoader<String, Timer>() {
                ...
                public Timer load(String method) throws Exception {
                    final MonitorConfig config = baseConfig.withAdditionalTag(new BasicTag("method", method));
