 ...
 */
public class StatsRecord extends AbstractMonitor<Long> implements CompositeMonitor<Long> {
    private static final ThreadFactory threadFactory = new ThreadFactoryBuilder()
            .setDaemon(true)
            .setNameFormat("StatsRecod-%d")
            .build();
    protected static final ScheduledExecutorService defaultExecutor = Executors.newScheduledThreadPool(1, threadFactory);
    private static final Logger LOGGER = LoggerFactory.getLogger(StatsRecord.class);

    protected final MonitorConfig baseConfig;
    private final Counter count;
    private final Callable<Void> scheduledExecuteStarter;
    private final Counter totalMeasurement;
     ...
     * the user.
     */
    public StatsRecord(final MonitorConfig config, final StatsConfig statsConfig, final ScheduledExecutorService executor,
    		final String totalTagStatName, final boolean shouldAutoStart, final Tag...additionalTags) {
        super(config);

        this.baseConfig = (additionalTags != null) ?
        		config.withAdditionalTags(new BasicTagList(ImmutableList.copyOf(additionalTags))) :
        		config;
        final Tag statTotal = Tags.newTag(STATISTIC, totalTagStatName);
        ...
        this.totalMeasurement = new BasicCounter(baseConfig.withAdditionalTag(statTotal));
        ...
        this.monitors = new ImmutableList.Builder<Monitor<?>>()
                ...
                .build();
        this.scheduledExecuteStarter = new Callable<Void>() {
        	public Void call() {
    ...
    /**
     * Superclasses can call this after initializing their member state in their constructors.  We want to avoid a race condition
     * where the statistics are computed and then
     */
    protected void startComputingStats() {
    	try {
    		this.scheduledExecuteStarter.call();
    	}
    	catch (Exception exc) {
    		LOGGER.error("could not start stats computation, exception =" + exc);
    	}
    }
    ...
    /**
     * Records a measurement and applies statistics to it.
     * @param measure - the measurement being analyzed.
     */
    public void record(long measure) {
        synchronized(updateLock) {
            cur.record(measure);
        }
        count.increment();
        totalMeasurement.increment(measure);
    }

    /**
     * Records a measurement and applies statistics to it, using a generic number
     * @param measure - the measurement being analyzed
     */
    public void record(Number measure) {
    	record(measure.longValue());
    }
    ...
    public Long getValue() {
        ...
        return n > 0 ? totalMeasurement.getValue() / n : 0L;
    }
    ...
    public boolean equals(Object obj) {
        if (obj == null || !(obj instanceof StatsRecord)) {
            return false;
        }

        final StatsRecord m = (StatsRecord) obj;
    ...
    /**
     * Get the total measurement recorded
     */
    public long getTotalMeasurement() {
        return totalMeasurement.getValue();
    }
}
